...
% clear
% 读取数据
 % data = xlsread('Lake Superior - Mean Water Level.xlsx', 'Sheet1', 'B2:M25');

%%
% 假设原始矩阵为matrix，包含m行n列

[m, n] = size(data); % 获取矩阵的行数和列数
data = reshape(data', 1, m*n); % 将矩阵转置后使用reshape函数
data=data';
%%
P=data(1:277,:)';
N=length(P);
n=263;%自己选取训练集个数*********《需要自己输入》**********
F = P(1:n+2);
...
legend("Actual Values", "Optimal Values", "Location", "best");

title('Comparison of Optimal and Actual Water Levels','FontSize', 22 , 'FontWeight', 'bold');
xlabel('Month', 'FontSize', 20);
ylabel('Water Level(m)', 'FontSize', 20);
